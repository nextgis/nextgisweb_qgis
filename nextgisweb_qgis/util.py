...
def sld_to_qml_raster(xml):
    ...
    for _channel in _raster_symbolizer.find("./ChannelSelection", namespaces=nsmap):
        ...
        rasterrenderer.attrib[f"{band}Band"] = _channel.find("./SourceChannelName", namespaces=nsmap).text

        if (_contrast_enhancement := _channel.find("./ContrastEnhancement", namespaces=nsmap)) is not None:
            if (_normalize_enhancement := _contrast_enhancement.find("./NormalizeEnhancement", namespaces=nsmap)) is not None:
                contrast_enhancement = E(f"{band}ContrastEnhancement")
                rasterrenderer.append(contrast_enhancement)
                for _vendor_option in _normalize_enhancement.findall("./VendorOption", namespaces=nsmap):
                    contrast_enhancement.append(E(_vendor_option.attrib["name"], _vendor_option.text))
